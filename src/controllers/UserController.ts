...
const createPgUser = async (req: Request, res: Response) => {
  const error = validationResult(req);
  if (!error.isEmpty()) {
    return res
      .status(statusCode.BAD_REQUEST)
  ...
  try {
    const userId = await UserService.createPgUser(userCreateDto);
    if (!userId)
      return res
        .status(statusCode.CONFLICT)
        .send(util.fail(statusCode.CONFLICT, message.DUPLICATED));
    ...
    res
      .status(statusCode.CREATED)
      .send(
        util.success(statusCode.CREATED, message.CREATED_USER_SUCCESS, userId)
      );
  } catch (error) {
    console.log(error);
    // 서버 내부 오류 발생
